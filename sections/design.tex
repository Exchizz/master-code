...
The final design is depicted in figure \ref{fig:design:overall}. \todo{Correct this}
\begin{figure}[H]
	\centering
...
\textit{R2: The \program{Historian} must be agnostic with respect to the streams' encoding} \\
...
\textit{R5: The \program{Historian} must be robust to stream packets arriving out-of-order or never arriving}
...
\textit{R6: The \hist{} should record and replay packets containing timing information about the streams.}
...
\textit{R7: The \program{Historian} must replay streams' packets in same order as they are recorded}
See R9
\textit{R8: The \program{Historian} must be able to replay streams and metadata in real-time, as they are recorded}
See R9
\textit{R9: The \program{Historian} must replay streams such that a processing-node cannot tell whether the stream is replayed or live}
...
\textit{R10: The \program{Historian} must support replaying the same stream multiple times}
As Both \program{tcpdump} and \program{rtpplay} saves recordings to files, they both support replaying multiple times.
\textit{R11: The \program{Historian} must not rely on specific hardware with specific properties}
None of \program{tcpdump} and \program{rtpplay} reply on specific hardware.


Given design requirement 4 and that \program{RTP Tools} are not in the Debian repository, and that \program{RTP Tools} does not support IPv6 packets, \program{tcpdump} has been chosen as the software for the \program{Historian}.
...
\textit{R2: The \program{Historian} must have an interface that allows requesting:}\\
\begin{itemize}
	...
	The \textbf{PLAY} method should be used to request which stream to replay, start and duration.
...
\end{itemize}
All of these parameters can be set/requested using the RTSP protocol. Using RTSP v1.0 the \textbf{RECORD}-method can be used to tell the \hist{} to record a particular stream. 

\textit{R3: The \program{Historian} must be able to discover streams using the presence mechanism} \\
