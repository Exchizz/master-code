...
\begin{figure}[H]
	...
	\includegraphics[width=\textwidth]{figures/sap-figure}
	 \caption{The figure shows a SAP packet is sent on a wellknown multicast group. The SDP payload then announces the multicast group where the RTP packets can be found.}\label{fig:design:sap}
\end{figure}
...
\textit{The \program{Publisher} and \program{Subscriber} must be agnostic to the stream's payload.}\\
\textit{Multiple \program{Subscribers} may receive from the same multicast group.}\\


\textit{The \program{Publishers} and \program{Subscribers} must implement a presence mechanism to know about other \program{Publishers} and \program{Subscribers}}.\\




The mechanism should have the following properties:
\begin{itemize}
	\item \textit{It must takes a finite maximum number of seconds for a \program{Publisher} or \program{subscriber} to know who is present in a stream. This maximum time must be a parameter of each node.}

	\item \textit{It must take a finite maximum number of seconds for a \program{Publisher} or \program{Subscriber} to detect whether a multicast group is in use, and if yes, by whom.}

	\item \textit{Nodes leaving a multicast group sends a ``bye''.}
	
	\item \textit{It must be robust to ``bye''-messages not arriving to all nodes.}

	\item \textit{It must handle nodes joining and leaving at any arbitrary time.}
	
	\item \textit{The \program{Publishers} must be able to find unused multicast groups single-handedly.}
\end{itemize}


The presence mechanism can be implemented using the RTCP protocol. As mention in section \ref{sec:design:rtcp}, RTCP includes a SDES packet, that allows for identification of senders and receivers in an RTP session. As it must take a maximum finite amounts of seconds to wait, the SDES  packet should be resent periodically. As the SDES packet is small(less than 100 bytes), no estimation of bandwidth is required. a This default retransmission period must be set to 1 s, as this is reasonable time to wait, however this must a parameter such that it can be changed by the user. RTCP also includes a BYE-packet, in order to inform other nodes that it is leaving a RTP session.  If a \program{Subscriber} or \program{Publisher} have not received a SDES packet from a \program{Publisher}/\program{Subscriber} for $2*\text{retransmission rate}$, it must be assumed that the \program{Subscriber}/\program{Publisher} is down and the BYE packet has been lost.

\begin{itemize}
	\item A RTCP bye packet must be sent when a \sub or \pub is leaving an RTP session
	\item A RTCP SDES packet must be sent every X second containing CNAME of the host and whether it is a publisher or subscriber
\end{itemize}

\myparagraph{Publisher}
\begin{itemize}
	\item A Session announcement must be sent every X second to announce an RTP session
\end{itemize}

\myparagraph{Subscriber}
\begin{itemize}
	\item A subscriber must randomly generate a multicast group
	\item If the multicast group is in use by another \pub, a new multicast group must be generated.
\end{itemize}

To detect whether a multicast group is in use, can be handled by monitoring the well known address for $2*\text{SDP retransmission period}$ s. If a \program{Publisher} wants to send a data stream to multicast group X, it should join the wellknown multicast group in order to see if another \program{Publisher} announces a RTP session on multicast group X. If no session announcement of multicast group X is received, the \program{Publisher} can start sending announcements and send data to multicast group X. If a session announcement for multicast group X is received, the \program{Publisher} must generate a new multicast group, Y, where it again starts check if multicast group Y is in use. This is depicted in figure \ref{fig:design:pubsub:inuse}.

\missingfigure{Flowchart of finding free multicast group}

\textit{Given the unique name of a stream, a \program{Subscriber} must be able to resolve the multicast address of the stream.}\\
The SDP format allows a unique, human readable session identifier in\textbf{s=}. The \program{Subscriber} should allow a parameter that allows setting an expression that is matched against the session name. If multiple matches are found, the \program{Subscriber} should inform the user that multiple matches exist and exit.


\textit{Both \program{Publishers} and \program{Subscribers} must be able to cope with nodes joining asynchronously.}\\
As session announcements are resent periodically, \program{Subscribers} and \program{Publishers} can join and leave at any time.

\textit{Essential and non-essential metadata must be sent to \program{Subscribers} periodically.}


\textit{The \pubs and \subs should only do work when 1) data is received and must be processed 2) when periodic work must be done.}
This requirement is similar to the general workflow of a \ac{GUI}, where code-blocks are executed when a button is clicked. These applications also usually implement timers such that code can be executed with a given interval. This is usually implemented in an event-driven manner. A method is attached to buttons and timers as callbacks, such that they can be run when the GUI either receives a click on a button from the user or the timer is invoked. \footnote{\url{https://en.wikipedia.org/wiki/Event-driven_programming}}

Given the nature of the \pubs and \subs, that they must only do work when either data is received or a timer says its time, the \pubs and \subs can be implemented in an event-driven manner.
...
In order to provide a clean UNIX interface to the \pro, a pipe should be used to provide the \pub with the source stream and non-essential/essential metadata. This should also be used between the \sub and \con. The pipes should be used, in order to generate a generic interface between \pro/\con and the \pub/\sub, respectively. 

In order to make the \con and \pro as generic and language independent as possible, the \pub and \sub should be the logical masters of the \con and \pro, respectively. This means, the \pub and \sub should start the \pro and \con, respectively, such that the \pro and \con are programs of any language that receives a pair of pipes they should read and write to. This is depicted in figure \ref{fig:design:pubsub:runmode_mastere}.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{figures/runmode_master}
	\caption{ok} \label{fig:design:pubsub:runmode_master}
\end{figure}

If the \pub and \sub where not the masters, but the \pro and \con where put in a pipe with \pub and \sub, respectively, the user had to enter information about the pipes, as this way to run the programs does not allow the programs to pass arguments to each other. This is depicted in figure \ref{fig:design:pubsub:runmode_pipeline}.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{figures/runmode_pipeline}
	\caption{ok} \label{fig:design:pubsub:runmode_pipeline}
\end{figure}
...
\subsection{Metadata profile} \label{sec:design:metadataprofile} \todo{Move to implementation?}
