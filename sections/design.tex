...
%\begin{enumerate}[{(a)}]	
	\item Programs should do one thing, and do it well. \label{req:des:well}
...
\begin{enumerate}[ref=R-D-\arabic* on P.\thepage]
    ...
    \item Default Linux tools and packages should be used if possible. \label{req:design:linux}
\end{enumerate}
...
Given design requirement X\todo{Fix requirement}, it has been decided to interface the \pubs{} and \subs{} with the \pros{} and \cons{} using pipes, as this loosely couples the two pairs of programs. Two pipes must be used, as the \pros{} and \subs{} must send a source stream and metadata to the \pubs{} and \cons, respectively.
...
\todo{Add RTCP SDES contains "tool" to say whether node is pub or sub}
...
\textit{R1: The \program{Historian} must support recording multiple streams to local mounted storage}\\
This can be implemented by both \program{tcpdump} and \program{rtpdump}. \\

\noindent\textit{R2: The \program{Historian} must be agnostic with respect to the streams' encoding} \\
...
\noindent\textit{R4: The \program{Historian} must replay streams with all of the streams' associated metadata}\\
In order for the replayed streams to be unambiguous, essential metadata must be replayed as well. As essential metadata are SDP packets encapsulated in RTP packets, both \program{tcpdump} and \program{rtpdump} support replaying streams unambiguously. Non-essential metadata is also RTP packets, so both programs supports non-essential metadata as well.\\

\noindent\textit{R5: The \program{Historian} must be robust to stream packets arriving out-of-order or never arriving}\\
...
\noindent\textit{R7: The \program{Historian} must replay streams' packets in same order as they are recorded}\\
See R9\\

\noindent\textit{R8: The \program{Historian} must be able to replay streams and metadata in real-time, as they are recorded}\\
See R9\\

\noindent\textit{R9: The \program{Historian} must replay streams such that a processing-node cannot tell whether the stream is replayed or live}\\
...
\noindent\textit{R10: The \program{Historian} must support replaying the same stream multiple times}\\
As Both \program{tcpdump} and \program{rtpplay} saves recordings to files, they both support replaying multiple times.\\

\noindent\textit{R11: The \program{Historian} must not rely on specific hardware with specific properties}\\
Neither of \program{tcpdump} nor \program{rtpplay} relies on specific hardware.\\

\noindent{}Given design requirement 4, that \program{RTP Tools} are not in the Debian repository, and that \program{RTP Tools} do not support IPv6 packets, \program{tcpdump} has been chosen as the software for the \program{Historian}.\\
\todo{Change tcpdump -> tcpdump/tcpreplay, rtpdump -> rtptools}
