...
\noindent \textit{R4: The Essential metadata must contain a timestamp that relates to the sample number. The precision of the timestamp must be within $0.33\mu$ seconds}\\
...
Given design requirement X, it has been decided to interface the \pubs and \subs with the \pros{} and \cons{} using named pipes. Two pipes must be used, as the \pros{} and \subs{} must send a source stream and metadata to the \pubs{} and \cons, respectively.

\subsubsection*{Runmode}
In order to decide how the \pubs{} and \subs{} should be run with the \pros{} and \cons{}, two ideas are compared.

The first idea is shown in figure \ref{fig:design:pubsub:runmode_pipeline}.
...
The design suggests the \pro{} and \pub{} are run in a pipeline such that the output of the \pro{} is fed as input to the \pub{}.
Furthermore, the output of the \sub{} is fed as input to the \con{}. Unfortunately this design does not permit the \pro{} and \pub{} nor \subs{} or \con{} to exchange information such as the name of the named pipe during startup.  This is depicted in figure \ref{fig:design:pubsub:runmode_pipeline}.

\begin{figure}[H]
	\centering
	\includegraphics[width=1\textwidth]{figures/runmode_master}
	\caption{The figures shows the \pub{} and \sub{} run the \pro{} and \con{} respectively. This design allows the \pubs{} to pass inforation to the \pro{} and \con{}} \label{fig:design:pubsub:runmode_master}
\end{figure}

The second design allows parameters to be passed from the \pub{}/\sub{} to the \pro{}/\con{}, respectively. In order to make the \con{} and \pro{} as generic and language independent as possible, the \pub{} and \sub{} should be the logical masters of the \con{} and \pro{}, respectively. This means, the \pub{} and \sub{} should start the \pro{} and \con{}, respectively, such that the \pro{} and \con{} are programs of any language that receives a pair of pipes they should read and write to.
...
\noindent From requirement 1, a presence mechanism must be design. This design is described in section \ref{sec:design:presencemechanism}. The design of the \pubs{} and \subs{} can be found in section \ref{sec:design:publisher} and \ref{sec:design:subscriber}, respectively.
...
\end{figure} \todo{Move to implementation?}


\myparagraph{Events}
Given that the \pub{} must be implement using an event loop, the \pub{} must at last implement the following events:

\todo{Mention SR packet, track timerequirement from usecase -> analysis -> design.}

\begin{itemize}
	\item When a packet is received from the wellknown multicast group.
