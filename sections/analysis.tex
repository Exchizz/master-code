...
Given design requirement X in section \ref{sec:analysis:designrequirements}, the \program{Publishers} and \program{Subscribers} must be opaque such that they encapsulate all of their internal protocols. In practice this means that the \program{Producers} and \program{Consumers} must be unaware of whether the communication between the \program{Subscribers} and \program{Publishers}.
...
As metadata is required, it must be possible to provide metadata to a \program{Publisher} such that it can stream it to the \program{Subscribers}.
...
\begin{enumerate}
	...
	\item A stream must be packet-oriented and stateless
	\item A stream must be transported over UDP
...
\begin{enumerate}
	...
	\item Multiple \program{Subscribers} may receive from the same multicast group.
	...
	\item The payload length of the stream should be adjustable as a parameter on the \program{Publishers}.
		\begin{enumerate}
			\item If the requested payload size exceeds practical limits, a warning must be given.
		\end{enumerate}
