...
If a \program{Publisher} publishes to a multicast group already in use, it must give a warning to the user. In order for this to work, the \program{Publishers} and \program{Subscribers} must implement a presence protocol that detects the presence of other \program{Publishers} and \program{Subscribers} on a specific stream. This must be implemented in a way such that possible conflicts can be detected within X seconds from where a \program{Publisher}/\program{Subscriber} is run, where X is specified as a parameter to the nodes of the system.
...
From use case \ref{sec:usecase:drone}, the \pubs{} and \subs{} must support being run on the same host. In order for the presence mechanism to know about all publishers and subscribers, each publisher and subscriber must be uniquely named and support having a sensible name set.
...
Given that some streams must not be recorded, the \program{Historian} must support ignoring streams that explicitly state not to be recorded. Network packets containing timestamps of the streams must be replayed as well.
...
From the analysis, the following requirements are extracted.
\subsection{Streams} \label{sec:analysis:streams}
\begin{enumerate}[ref=R-A-P-\arabic* on P.\thepage]
    ...
    \item Streams must be sent in a session. \label{req:a:stream:session}
    \item Each stream must have its own session. \todo{But one group might have multiple streams} \label{req:a:stream:streamtosession}
...
\begin{enumerate}
	...
	\item The \pubs{} and \subs{} should only do work when 1) data is received and must be processed; 2) periodic work must be done.
\end{enumerate}


\subsubsection*{Publisher}
\begin{enumerate}[ref=R-A-P-\arabic* on P.\thepage]
	\item The \pubs{} must be able to publish to a topic. \label{req:a:pub:publishtopic}
...
\begin{enumerate}
	...
	\item The \sub{} must be transparent from the \program{Consumer's} perspective.
