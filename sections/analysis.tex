...
As in use case \ref{sec:usecase:drone} where a single RPI is used, the \program{Historian} must run on the same \ac{RPi} as the \ac{MCLURS} software. In order for this to work, the multicast traffic must also work on  a virtual interface.
