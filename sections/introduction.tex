...
As there is no guarantee that all batboxes are powered up when the historian starts recording, the historian must be able to autodiscover batboxes, and add them to the list of streams to record. In order for this to work, a topic should be reserved for batboxes to announce their existence, from where the historian can listen. 

When a batbox is powered up, it should send a message containing its name, a descriptive name and the name of the topics that it publishes to.
