...
\program{tcpdump} of the RTP has been omitted, but the is found as with the SDP packet in \ref{sec:verify:sessionannouncement}.
...
\todo{add essential metadata as goal to first test}
...
This test verifies samples from \program{Snapshot} are sent and received correctly by the \con{}.
The test is conducted by running a \pub{} on a \ac{RPi}, and a \sub{} on a laptop. The laptop and the \ac{RPi} is connected over ethernet. 100 packets are sent in chunks of 4096 bytes\footnote{The default pagesize in linux}. The test is conducted by intersecting the pipe using \program{tee} between \program{Snapchat} and the \program{Publisher}, such that the samples from \program{Snapchat} can be saved to a file and sent by the \pub{}.\\

\noindent The \con{} has been made such that it writes all data it receives to a local file on a laptop. 
The two files are compared using \program{sha256sum}, which is prone to packets that have arrived out of order or not arrived at all: \\
...
Listing \ref{lst:implementation:uniquepub1} shows the output from the first \program{Publisher}. It should be noted that it waits for 5 seconds, and then continues starting up.
